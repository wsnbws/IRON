...
@PIPELINES.register_module()
class PhotoMetricDistortionMulti(object):
    """Apply photometric distortion to multiple images consistently.

    Each transformation is applied with a probability of 0.5. The position of
    random contrast is in second or second to last. The same random parameters
    are applied to all images in the input list.
    ...
    def __call__(self, results):
        ...
        """
        assert 'imgs' in results, "PhotoMetricDistortionMulti requires 'imgs' in results"
        
        # Once determined, same transform is applied to all frames
        # Randomly determine params for brightness
        apply_brightness = random.randint(0,1)
        if apply_brightness:
            brightness_beta = random.uniform(-self.brightness_delta, self.brightness_delta)
        
        # Randomly determine params for contrast
        contrast_mode = random.randint(0,1)
        apply_contrast = random.randint(0,1)
        if apply_contrast:
            contrast_alpha = random.uniform(self.contrast_lower, self.contrast_upper)
            
        # Randomly determine params for saturation
        apply_saturation = random.randint(0,1)
        if apply_saturation:
            saturation_alpha = random.uniform(self.saturation_lower, self.saturation_upper)

        # Randomly determine params for hue
        apply_hue = random.randint(0,1)
        if apply_hue:
            hue_delta_val = random.randint(-self.hue_delta, self.hue_delta)
        ...
        for img in results['imgs']:
            ...
            # Brightness
            if apply_brightness:
                img = np.clip(img + brightness_beta, 0, 255)

            # Contrast (mode 1)
            if contrast_mode == 1 and apply_contrast:
                img = np.clip(img * contrast_alpha, 0, 255)

            # Saturation
            if apply_saturation:
                img = mmcv.bgr2hsv(img)
                img[:, :, 1] = np.clip(img[:, :, 1] * saturation_alpha, 0, 255)
                img = mmcv.hsv2bgr(img)

            # Hue
            if apply_hue:
                img = mmcv.bgr2hsv(img)
                img[:, :, 0] = (img[:, :, 0].astype(int) + hue_delta_val) % 180
                img = mmcv.hsv2bgr(img)
            
            # Contrast (mode 0)
            if contrast_mode == 0 and apply_contrast:
                img = np.clip(img * contrast_alpha, 0, 255)

            distorted_imgs.append(img.astype(np.uint8))

        results['imgs'] = distorted_imgs
        results['img'] = distorted_imgs[-1]
        return results
