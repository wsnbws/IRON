...
class EncoderDecoderSTATIC(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EncoderDecoderSTATIC, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(EncoderDecoderSTATIC, self).init_weights(pretrained)
    ...
    def _decode_head_forward_train(self, x, gt_semantic_seg):
        ...
        losses = dict()
        loss_decode = self.decode_head.forward_train(x, gt_semantic_seg)    
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        final_losses = dict()
        img_feature = self.extract_feat(img)
        loss_decode_t = self._decode_head_forward_train(img_feature, gt_semantic_seg)
        for k, v in loss_decode_t.items():
            final_losses[k] = final_losses.get(k, 0) + v

        if self.with_auxiliary_head:
            loss_aux_t = self._auxiliary_head_forward_train(img_feature, img_metas, gt_semantic_seg)
