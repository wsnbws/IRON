...
from .untils import get_clones, LayerNorm2d, DropPath
...
class Fuser(nn.Module):
    def __init__(self, layer=None, num_layers=2, dim=None, input_projection=False):
        super().__init__()
        self.proj = nn.Identity()
        if layer is None:
            assert dim is not None, "dim must be provided when layer is None"
            layer = CXBlock(
                dim=dim,
                kernel_size=7,
                padding=3,
...
class MemoryEncoder(nn.Module):
    def __init__(
        ...
        # For the case where masks already match pix_feat spatial size
        self.mask_downsampler = MaskDownSampler(total_stride=total_stride)
