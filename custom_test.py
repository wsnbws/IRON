...
def resize_and_pad_gt(gt_seg, img_meta):
    """
    根据 img_meta 中的预处理信息，对 GT 进行 resize 和 pad，
    使其与模型输出 (seg_pred) 对齐。
    """
    # 强制固定参数 (Resize to max_h=360, max_w=2048, keep_ratio=True)
    target_max_w = 2048
    target_max_h = 360
    pad_h = 512
    pad_w = 640
    ...
    # 获取原始尺寸
    ori_h, ori_w = gt_seg.shape[:2]
    
    # 计算 Resize 后的尺寸 (keep_ratio=True)
    scale = min(target_max_w / ori_w, target_max_h / ori_h)
    new_w = int(ori_w * scale)
    new_h = int(ori_h * scale)
    
    # Resize GT (使用最近邻插值)
    gt_seg = mmcv.imresize(gt_seg, (new_w, new_h), interpolation='nearest')
    
    # Pad
    padded_gt = np.full((pad_h, pad_w), 255, dtype=gt_seg.dtype)
    
    # 默认左上角对齐
    # 加上边界检查，防止 new_h/new_w 超过 pad_h/pad_w
    valid_h = min(new_h, pad_h)
    valid_w = min(new_w, pad_w)
    padded_gt[:valid_h, :valid_w] = gt_seg[:valid_h, :valid_w]
    ...
    for i, data_batch in enumerate(data_loader):
        ...
        if eval_mode and evaluator is not None:
            ...
            for img_meta in img_metas[0]:
                ...
                # 对 GT 进行 Resize 和 Pad 以对齐预测值
                gt_seg = resize_and_pad_gt(gt_seg, img_meta)
                
                batch_gts.append(gt_seg)
            
            batch_gts = np.stack(batch_gts, axis=0)
            # print(f"batch_gts: {batch_gts.shape}")
...
def main():
    ...
    if args.eval:
        ...
        logger.info(f'overall_acc: {eval_metrics["overall_acc"]:.4f}')
