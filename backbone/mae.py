...
from mmcv_custom import load_mae_checkpoint
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, -1)    # (B, num_heads, N, C//num_heads) ---> (B, N, num_heads, C//num_heads)  ---> (B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class MAE(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[3, 5, 7, 11], fpn1_norm='SyncBN', frozen_backbone=False):
        super().__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            load_mae_checkpoint(self, pretrained, strict=False, logger=logger)
