...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, hw=None):
        B, N, C = x.shape
        ...
        if self.relative_position_bias_table is not None:
            if (not self.training) and hw is not None and self.window_size is not None and \
               (hw[0] * hw[1] == N) and ((hw[0], hw[1]) != tuple(self.window_size)):
                # Interpolate bias table to new window size (Hp, Wp) without modifying parameters
                device = self.relative_position_bias_table.device
                dtype = self.relative_position_bias_table.dtype
                Hp, Wp = int(hw[0]), int(hw[1])
                Wh0, Ww0 = int(self.window_size[0]), int(self.window_size[1])

                # reshape to 2D grid and interpolate
                bias_2d = self.relative_position_bias_table.view(2 * Wh0 - 1, 2 * Ww0 - 1, self.num_heads)
                bias_2d = bias_2d.permute(2, 0, 1).unsqueeze(0).to(device=device, dtype=dtype)  # 1, nH, Hrel, Wrel
                bias_2d_resized = F.interpolate(
                    bias_2d,
                    size=(2 * Hp - 1, 2 * Wp - 1),
                    mode='bicubic',
                    align_corners=False
                )  # 1, nH, 2Hp-1, 2Wp-1
                bias_2d_resized = bias_2d_resized.squeeze(0)  # nH, 2Hp-1, 2Wp-1

                # compute new relative position indices for (Hp, Wp)
                coords_h = torch.arange(Hp, device=device)
                coords_w = torch.arange(Wp, device=device)
                coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Hp, Wp
                coords_flatten = torch.flatten(coords, 1)  # 2, Hp*Wp
                relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, N, N
                relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # N, N, 2
                relative_coords[:, :, 0] += Hp - 1
                relative_coords[:, :, 1] += Wp - 1
                relative_coords[:, :, 0] *= (2 * Wp - 1)
                relative_position_index_new = relative_coords.sum(-1)  # N, N

                bias_flat = bias_2d_resized.permute(1, 2, 0).contiguous().view((2 * Hp - 1) * (2 * Wp - 1), self.num_heads)
                relative_position_bias = bias_flat[relative_position_index_new.view(-1)].view(N, N, self.num_heads)
...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, hw=None):
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, hw=hw))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, hw=hw))
...
class ConvMAE(nn.Module):
    ...
    def __init__(self, img_size=[512, 128, 64], patch_size=[4,2,2], in_chans=3, num_classes=80, embed_dim=[256,384,768], depth=[2,2,11],
                 num_heads=12, mlp_ratio=[4,4,4], qkv_bias=True, qk_scale=None, drop_rate=0.1, attn_drop_rate=0,
    ...
    def forward_features(self, x):
        ...
        if self.pos_embed is not None:
            if (not self.training):
                # Use temporary interpolated pos embed for eval if size changed
                Hp0, Wp0 = self.patch_embed3.patch_shape
                if Hp * Wp == self.pos_embed.shape[1] and (Hp, Wp) == (Hp0, Wp0):
                    x = x + self.pos_embed
                else:
                    pos = self.pos_embed
                    pos_2d = pos.view(1, Hp0, Wp0, -1).permute(0, 3, 1, 2)  # 1, C, Hp0, Wp0
                    pos_2d = F.interpolate(pos_2d, size=(Hp, Wp), mode='bicubic', align_corners=False)
                    pos_interp = pos_2d.permute(0, 2, 3, 1).reshape(1, Hp * Wp, -1)
                    x = x + pos_interp
            else:
                x = x + self.pos_embed
    ...
    def forward(self, x):
        # 简单图片可视化：在 forward 一开始执行一次，仅需传入 img 与 mean/std
        # if not hasattr(self, '_debug_img_saved') or (self._debug_img_saved is False):
        #     mean = np.array([108.375, 108.375, 108.375], dtype=np.float32)
        #     std = np.array([50.7195, 50.7195, 50.7195], dtype=np.float32)
        #     # 若输入为 (B*T,C,H,W)，需要提供 num_frames；默认按 3 帧
        #     nframes = 3
        #     save_images_simple(x, mean=mean, std=std, to_rgb=True,
        #                        out_dir='/public/home/wangshuo/DrivableTask/out/DrivableSeg/debug_gt',
...
def _denorm_to_uint8(img_chw: torch.Tensor, mean: np.ndarray, std: np.ndarray, to_rgb: bool = True) -> np.ndarray:
    """
    Convert a normalized CHW tensor to uint8 HWC in BGR order for saving.
    """
    x = img_chw.detach().cpu().numpy()  # C,H,W
    x = x.transpose(1, 2, 0)  # H,W,C
    x = x * std.reshape(1, 1, -1) + mean.reshape(1, 1, -1)
    if to_rgb:
        x = x[:, :, ::-1]  # RGB -> BGR for imwrite
    x = np.clip(x, 0, 255).astype(np.uint8)
    return x


def save_images_simple(imgs: torch.Tensor, mean, std, to_rgb: bool = True,
                       out_dir: str = '/public/home/wangshuo/DrivableTask/out/DrivableSeg/debug_gt',
                       max_samples: int = 1, num_frames: int = None, prefix: str = None) -> None:
    """
    简化版图片保存，仅依赖 imgs 与 mean/std。
    支持 imgs 形状：(B,T,C,H,W) 或 (B*T,C,H,W)。
    """
    assert imgs.dim() in (4, 5)
    if imgs.dim() == 5:
        ...
        b = bt // num_frames
        t = num_frames
        clip = imgs.view(b, t, c, h, w)

    mmcv.mkdir_or_exist(out_dir)
    mean = np.array(mean, dtype=np.float32)
    std = np.array(std, dtype=np.float32)

    for i in range(min(max_samples, clip.shape[0])):
        ts = f"{time.time():.3f}".replace('.', '')
        pre = prefix if prefix is not None else f"sample{i}_{ts}"
        # 取 T, T-1, T-2（若 t<3 则尽量取最后一帧与前一帧）
        idxs = [-1, -2 if t >= 2 else -1, -3 if t >= 3 else -1]
        labels = ['cur', 'tm1', 'tm2']
        for idx, lab in zip(idxs, labels):
            img = _denorm_to_uint8(clip[i, idx], mean, std, to_rgb)
            mmcv.imwrite(img, f"{out_dir}/{pre}_img_{lab}.png")
